...
fig, ax, pl = scatter(M, pts, markersize = 20, color=:green)
arrows3d!(
...
fig, ax, pl = geodesics(M, pts; linewidth = 2, color = :blue)
pts2 = HyperboloidPoint.([[2.0, 2.0, 3.0], [2.0, -2.0, 3.0], [2.5, 0.0, sqrt(7.25)]])
scattergeodesics!(ax, M, pts2; linewidth = 3, color = :green, markersize = 16, closed = true)
fig
```

## The Poincaré disk and ball

For the [Poincaré disc](https://en.wikipedia.org/wiki/Poincaré_disk_model) we can plot 2D and 3D hyperbolic data.
